...
//
// PORT — Doit rester synchronisé avec batiflow-web/lib/debourses-calc.ts.
// Toute évolution de la logique de calcul doit être répercutée dans les
// deux fichiers (Node/Next.js et Deno/Edge Function).
...
export interface GlobalInputs {
  ...
  hourdis_m2: number;
  long_planche: number;
  hauteur_montee: number;
  montes_par_sac: number;
  camion_sable: number;
  camion_g0525: number;
  camion_g1525: number;
...
export interface Step7Inputs {
  ht_murs: number;
}
export interface Step8Inputs {
  larg: number;
  ht: number;
  dosage: number;
  n_barres: number;
...
export interface Step14Inputs {
  esp: number;
}
export interface Step15Inputs {
  larg_dall: number;
  ep: number;
  dosage: number;
}
...
export function computeRecap(inp: AllInputs): DebourseRecap {
  ...
  const tot_cim =
    ...
    cim(s8_vb, s8.dosage) +
    cim(s9_vb, s9.dosage) +
    cim(s11_vb, s11.dosage) +
    cim(s12_vb, s12.dosage) +
    cim(s13_vb, s13.dosage) +
    cim(s15_vb, s15.dosage);
