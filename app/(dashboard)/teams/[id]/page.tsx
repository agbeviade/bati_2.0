...
export default async function TeamDetailPage({
  ...
  if (tab === "members") {
    ...
    const availableUsers = (
      (companyUsers ?? []) as Pick<User, "id" | "full_name" | "role">[]
  ...
  if (memberIds.length > 0) {
    ...
    for (const entry of (openData ?? []) as Pick<
      Attendance,
      "id" | "user_id" | "check_in" | "project_id"
    >[]) {
      const m = members.find((m) => m.user_id === entry.user_id);
      if (m)
        m.openAttendance = { id: entry.id, check_in: entry.check_in, project_id: entry.project_id };
    }
  }
  ...
  return (
    <PageShell team={team}>
      <AttendanceTab teamId={id} members={members} projects={projects} history={history} />
    </PageShell>
  );
}
    ...
    <div className="max-w-2xl space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="truncate text-2xl font-bold">{team.name}</h2>
        </div>
