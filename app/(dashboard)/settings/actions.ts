...
export async function uploadCompanyAsset(
  ...
  if (!allowed.includes(file.type))
    return { url: null, error: "Format non supporté (JPG, PNG, WebP, SVG)." };
  ...
  const {
    data: { publicUrl },
  } = admin.storage.from("company-assets").getPublicUrl(path);
...
export async function removeCompanyAsset(type: "header" | "footer"): Promise<{ error?: string }> {
  const { companyId, supabase } = await getAuthedProfile();
