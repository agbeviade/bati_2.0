...
const TRANSITIONS: Record<QuoteStatus, { next: QuoteStatus; label: string }[]> = {
  ...
  sent:     [{ next: "approved", label: "Approuver" }, { next: "rejected", label: "Refuser" }],
  approved: [],
  ...
  return (
    <div className="flex gap-2 flex-wrap">
      {actions.map(({ next, label }) => (
        <Button
          key={next}
          variant={next === "approved" ? "default" : next === "rejected" ? "destructive" : "outline"}
          size="sm"
          disabled={isPending}
          onClick={() =>
      ...
      {currentStatus === "approved" && (
        <Button variant="outline" size="sm" asChild>
          <Link href={`/invoices/new?quote_id=${quoteId}`}>
            Créer une facture
          </Link>
        </Button>
      )}
