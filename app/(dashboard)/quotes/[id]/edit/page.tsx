...
export default function EditQuotePage() {
  ...
  const allItemsTotal =
    existingItems.reduce((s, i) => s + i.total, 0) +
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSaveMeta} className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="space-y-1.5 md:col-span-2">
              ...
              <textarea
                ...
                className="border-input bg-background focus-visible:ring-ring flex min-h-[60px] w-full resize-none rounded-md border px-3 py-2 text-sm focus-visible:ring-2 focus-visible:outline-none"
              />
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lignes existantes</CardTitle>
        </CardHeader>
            ...
            existingItems.map((item, i) => (
              <div key={item.id}>
                <div className="flex items-center gap-3 py-2 text-sm">
                  ...
                  <span className="w-24 flex-shrink-0 text-right font-medium">
                    {fmt(item.total)}
                  </span>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            onClick={() => setNewItems((prev) => [...prev, newLine()])}
          >
        ...
        <CardContent className="space-y-3">
          {newItems.length === 0 ? (
            <p className="text-muted-foreground py-2 text-center text-sm">
              Cliquez sur "Ajouter une ligne" pour compléter le devis.
            </p>
          ) : (
            <>
              {newItems.map((item) => (
                ...
                >
                  <select
                    value={item.category}
                    onChange={(e) => updateNew(item.id, "category", e.target.value)}
                  ...
                  <Input
                    ...
                    value={item.label}
                    onChange={(e) => updateNew(item.id, "label", e.target.value)}
                    className="h-9"
                  />
                  <Input
                    type="number"
                    min="0"
                    value={item.quantity}
                    onChange={(e) => updateNew(item.id, "quantity", e.target.value)}
                    className="h-9 text-right"
                  />
                  <Input
                    placeholder="u"
                    value={item.unit}
                    onChange={(e) => updateNew(item.id, "unit", e.target.value)}
                    className="h-9"
                  />
                  <Input
                    type="number"
                    ...
                    onChange={(e) => updateNew(item.id, "unit_price", e.target.value)}
                    className="h-9 text-right"
                  />
                  <div className="flex h-9 items-center justify-end px-2 text-sm font-medium tabular-nums">
