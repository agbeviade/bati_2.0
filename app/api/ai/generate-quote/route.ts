...
import { getAI, AI_MODEL, QUOTE_SYSTEM_PROMPT, GENERATE_QUOTE_TOOL, extractToolResult } from "@/lib/ai/client";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await recordAiUsage({
      companyId, userId: user.id, kind: "quote_from_description",
      model: AI_MODEL, startedAt, error: e,
    });
  ...
  await recordAiUsage({
    companyId, userId: user.id, kind: "quote_from_description",
    model: AI_MODEL, startedAt, response,
  });
