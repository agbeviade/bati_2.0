    ...
    <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 32px" }}>
      ...
      >
        ...
        <div style={{ textAlign: "right" }}>
          ...
          {quote.valid_until && (
            <p style={{ margin: "2px 0 0", color: "#555", fontSize: 13 }}>
              Valide jusqu'au {fmtDate(quote.valid_until)}
            </p>
          )}
        </div>
      </div>
            ...
            <tbody>
              {catItems.map((item, i) => (
                <tr key={item.id} style={{ background: i % 2 === 0 ? "white" : "#fafafa" }}>
                  ...
                  <td
                    style={{ padding: "6px 8px", borderBottom: "1px solid #f1f5f9", color: "#666" }}
                  >
                    {item.unit}
                  </td>
                  <td
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
        <div style={{ width: 280 }}>
          {[
            ...
            ...(quote.tax_rate > 0
              ? [{ label: `TVA (${quote.tax_rate}%)`, value: fmt(quote.tax_amount, currency) }]
              : []),
            ...(quote.margin_pct > 0
              ? [
                  {
                    label: `Marge (${quote.margin_pct}%)`,
                    value: fmt((quote.subtotal * quote.margin_pct) / 100, currency),
                  },
                ]
              : []),
          ].map((r) => (
          ...
          >
            <span>Total TTC</span>
            <span style={{ color: "#2563eb" }}>{fmt(quote.total, currency)}</span>
          </div>
        </div>
      </div>
